...
interface Paginated { items: Trailer[]; pagination: { page: number; pageSize: number; total: number; totalPages: number } }
...
export default function TrailersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(await apiFetch<Paginated>(`/trailers?${params}`, { token }));
  ...
  async function removeTrailer(id: string) {
    if (!confirm('¿Eliminar esta unidad por completo? Esta acción no se puede deshacer.')) return;
    try { await apiFetch(`/trailers/${id}`, { method: 'DELETE', token }); void load(); }
      ...
      <section className="flex-1 space-y-6">
        <header className="rounded-2xl border border-black/10 bg-steel p-6 text-sand shadow-panel flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold">Trailers / Unidades</h1>
          </div>
          <button onClick={() => setShowNew(true)} className="rounded-xl bg-moss px-5 py-2.5 text-sm font-semibold text-sand hover:bg-moss/80 transition">+ Nueva unidad</button>
        </header>
          ...
          {loading ? <div className="px-6 py-8 text-center text-sm text-ink/50">Cargando...</div> : (
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-black/5">
                {!data?.items.length && <tr><td colSpan={9} className="px-4 py-8 text-center text-ink/40">Sin unidades registradas.</td></tr>}
                {data?.items.map(t => (
                  <tr key={t.id} className="hover:bg-sand/10 transition">
                    <td className="px-4 py-3 font-medium">{t.economicNumber}</td>
                    <td className="px-4 py-3 text-ink/70">{t.plates ?? '—'}</td>
                    <td className="px-4 py-3 text-ink/70">{t.brand ?? '—'}</td>
                    <td className="px-4 py-3 text-ink/70">{t.model ?? '—'}</td>
                    <td className="px-4 py-3 text-ink/70">{t.color ?? '—'}</td>
                    <td className="px-4 py-3 text-ink/70">{t.mileage != null ? t.mileage.toLocaleString('es-MX') : '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${t.status === 'ACTIVE' ? 'bg-moss/20 text-moss' : 'bg-black/10 text-ink/50'}`}>
                        {t.status === 'ACTIVE' ? 'Activa' : 'Inactiva'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-ink/60">{t.createdAt.slice(0, 10)}</td>
                    <td className="px-4 py-3 flex gap-2">
                      <button onClick={() => setEditing(t)} className="text-xs text-steel hover:text-ink underline">Editar</button>
                      <button onClick={() => void removeTrailer(t.id)} className="text-xs text-alert hover:text-alert/70 underline">Eliminar</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function TrailerModal({ token, trailer, onClose, onSaved }: {
  token: string | null; trailer: Trailer | null;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (trailer) {
        await apiFetch(`/trailers/${trailer.id}`, { method: 'PATCH', token, body });
      } else {
        await apiFetch('/trailers', { method: 'POST', token, body });
      }
      onSaved();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-panel w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/10">
          <h2 className="font-bold text-ink text-lg">{trailer ? 'Editar unidad' : 'Nueva unidad'}</h2>
        ...
        <form ref={formRef} onSubmit={e => void submit(e)} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input name="economicNumber" required defaultValue={trailer?.economicNumber}
          ...
          <div>
            ...
            <input name="plates" defaultValue={trailer?.plates ?? ''}
              className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss/50" />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="color" defaultValue={trailer?.color ?? ''}
              className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss/50" />
          </div>
          <div>
            <label className="block text-xs font-medium text-ink/70 mb-1">Kilómetros</label>
            <input name="mileage" type="number" min="0" defaultValue={trailer?.mileage ?? ''}
              className="w-full rounded-lg border border-black/15 px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss/50" />
          </div>
