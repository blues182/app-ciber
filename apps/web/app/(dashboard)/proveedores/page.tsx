...
interface Provider { id: string; businessName: string; serviceType: string; phone: string | null; email: string | null; status: 'ACTIVE' | 'INACTIVE'; createdAt: string }
interface Paginated { items: Provider[]; pagination: { page: number; pageSize: number; total: number; totalPages: number } }
...
export default function ProveedoresPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(await apiFetch<Paginated>(`/providers?${params}`, { token }));
  ...
  async function deactivate(id: string) {
    ...
    try { await apiFetch(`/providers/${id}`, { method: 'DELETE', token }); void load(); }
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-[1400px] gap-6 p-6 lg:p-8">
      ...
      <section className="flex-1 space-y-6">
        <header className="rounded-2xl border border-black/10 bg-steel p-6 text-sand shadow-panel flex items-center justify-between">
          ...
          <button onClick={() => setShowNew(true)} className="rounded-xl bg-moss px-5 py-2.5 text-sm font-semibold text-sand hover:bg-moss/80 transition">+ Nuevo proveedor</button>
        </header>
          ...
          {loading ? <div className="px-6 py-8 text-center text-sm text-ink/50">Cargando...</div> : (
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-black/5">
                {!data?.items.length && <tr><td colSpan={6} className="px-4 py-8 text-center text-ink/40">Sin proveedores registrados.</td></tr>}
                {data?.items.map(p => (
                  <tr key={p.id} className="hover:bg-sand/10 transition">
                    <td className="px-4 py-3 font-medium">{p.businessName}</td>
                    <td className="px-4 py-3 text-ink/70">{p.serviceType}</td>
                    <td className="px-4 py-3 text-ink/70">{p.phone ?? '—'}</td>
                    <td className="px-4 py-3 text-ink/70">{p.email ?? '—'}</td>
                    <td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${p.status === 'ACTIVE' ? 'bg-moss/20 text-moss' : 'bg-black/10 text-ink/50'}`}>{p.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}</span></td>
                    <td className="px-4 py-3 flex gap-2">
                      <button onClick={() => setEditing(p)} className="text-xs text-steel hover:text-ink underline">Editar</button>
                      {p.status === 'ACTIVE' && <button onClick={() => void deactivate(p.id)} className="text-xs text-alert hover:text-alert/70 underline">Desactivar</button>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {(showNew || editing) && (
        <ProviderModal token={token} provider={editing}
...
function ProviderModal({ token, provider, onClose, onSaved }: { token: string | null; provider: Provider | null; onClose: () => void; onSaved: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (provider) { await apiFetch(`/providers/${provider.id}`, { method: 'PATCH', token, body }); }
      else { await apiFetch('/providers', { method: 'POST', token, body }); }
      onSaved();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-panel w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/10">
          <h2 className="font-bold text-ink text-lg">{provider ? 'Editar proveedor' : 'Nuevo proveedor'}</h2>
        ...
        <form ref={formRef} onSubmit={e => void submit(e)} className="px-6 py-5 space-y-4">
          ...
          {[
            { label: 'Razón social *', name: 'businessName', type: 'text', required: true, def: provider?.businessName },
            { label: 'Tipo de servicio *', name: 'serviceType', type: 'text', required: true, def: provider?.serviceType },
            { label: 'Teléfono', name: 'phone', type: 'text', required: false, def: provider?.phone ?? '' },
            { label: 'Correo', name: 'email', type: 'email', required: false, def: provider?.email ?? '' },
          ].map(f => (
            <div key={f.name}>
